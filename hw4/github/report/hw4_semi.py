	...
	if 'training_label' in path:
		with open(path, 'r') as f:
			...
			'''for i in range(len(lines)):
				...
				lines[i] = ''.join(c for c in lines[i] if c not in punctuation)'''
			#print(lines)
...
#if = pd.read_csv('./train_nolabel.csv')
#y_nolabel = f['label'].values
...
X_train, X_val, y_train, y_val = train_x[:10000], train_x[10000:20000], y[:10000], y[10000:20000]
...
train_nolabel_dataset = TwitterDataset(X=train_x_no_label, y=None)
train_nolabel_loader = torch.utils.data.DataLoader(dataset = train_nolabel_dataset,
											batch_size = batch_size,
											shuffle = False,
											num_workers = 8)

def labeling(batch_size, model, train_nolabel_loader, device):
	pos_threshold = 0.85
	model.eval()
	ret_output = []
	x_nolabel = []
	y_nolabel = []
	with torch.no_grad():
		for i, inputs in enumerate(train_nolabel_loader):
			inputs = inputs.to(device, dtype=torch.long)
			outputs = model(inputs)
			outputs = outputs.squeeze()
			for k in range(inputs.shape[0]):
				...
				if label > pos_threshold:
					x_nolabel.append(list(sen))
					y_nolabel.append(1)
				if label < 1-pos_threshold:
					x_nolabel.append(list(sen))
					y_nolabel.append(0)
	x_nolabel = np.array(x_nolabel, dtype = "float")
	y_nolabel = np.array(y_nolabel, dtype = "float")
	print('labeled data: {}'.format(len(x_nolabel)))
	return x_nolabel, y_nolabel

preprocess = Preprocess(train_x_no_label, sen_len, w2v_path=w2v_path)
embedding = preprocess.make_embedding(load=True)
train_x_nolabel = preprocess.sentence_word2idx()
train_nolabel_dataset = TwitterDataset(X=train_x_no_label, y=None)
train_nolabel_loader = torch.utils.data.DataLoader(dataset = train_nolabel_dataset,
											batch_size = batch_size,
											shuffle = False,
...
train_x_nolabel, y_nolabel = labeling(batch_size, model, train_nolabel_loader, device)

train_all = np.concatenate((X_train, train_x_nolabel), axis=0)
y_all = np.concatenate((y_train, y_nolabel), axis=0)
print(train_all.shape, y_all.shape)


model = LSTM_Net(embedding, embedding_dim=250, hidden_dim=150, num_layers=1, dropout=0.5, fix_embedding=fix_embedding)
model = model.to(device) # device為 "cuda"，model 使用 GPU 來訓練（餵進去的 inputs 也需要是 cuda tensor）

new_train_dataset = TwitterDataset(X=torch.LongTensor(train_all), y=torch.LongTensor(y_all))
new_train_loader = torch.utils.data.DataLoader(dataset = new_train_dataset,
											batch_size = batch_size,
											shuffle = False,
											num_workers = 8)
training(batch_size, epoch, lr, model_dir, new_train_loader, val_loader, model, device)
...
print("save csv ...")
tmp.to_csv(os.path.join(path_prefix, 'rnn.csv'), index=False)
