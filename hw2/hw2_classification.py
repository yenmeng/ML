...
sig_cols = ['capital gains', 'capital losses', 'dividends from stocks']
big_top_value_cols = [col for col in X_train.columns if X_train[col].value_counts(dropna=False, normalize=True).values[0] > 0.999]

for item in numeric_cols:
    name = item + str('_1')
    X_train[name] = X_train[item]**2
    X_test[name] = X_test[item]**2
for item in numeric_cols:
    name = item + str('_2')
    X_train[name] = X_train[item]**3
    X_test[name] = X_test[item]**3
# drop country
#X_train = X_train.drop(big_top_value_cols, axis=1)
#X_test = X_test.drop(big_top_value_cols, axis=1)
...
def _cross_entropy_loss(y_pred, Y_label, w):
    ...
    cross_entropy = -np.dot(Y_label, np.log(y_pred)) - np.dot((1 - Y_label), np.log(1 - y_pred)) + Lambda/2 * np.dot(w,w)
...
for epoch in range(max_iter):
    ...
    for idx in range(int(np.floor(train_size / batch_size))):
        ...
        w = w * (1 - 2 * learning_rate * Lambda) - learning_rate/np.sqrt(step) * w_grad
