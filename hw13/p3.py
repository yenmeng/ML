...
def MAML(model, optimizer, x, n_way, k_shot, q_query, loss_fn, inner_train_step = 1, inner_lr = 0.4, train = True, create_graph = True):
  """
  Args:
 ...
 _query: num of images to update in each class when testing
  ...
  for meta_batch in x:
    ...
    for inner_step in range(inner_train_step): 
      ...
      grads = torch.autograd.grad(loss, fast_weights.values(), create_graph = create_graph)
...
for epoch in range(max_epoch):
  ...
  for step in tqdm(range(len(train_loader) // (meta_batch_size))): 
    ...
    if epoch < 50:
        meta_loss, acc = MAML(meta_model, optimizer, x, n_way, k_shot, q_query, loss_fn, create_graph = False)
    else:
        meta_loss, acc = MAML(meta_model, optimizer, x, n_way, k_shot, q_query, loss_fn, create_graph = True)
