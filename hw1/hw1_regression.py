...
"""# **Preprocessing**
    ...
    """
indexNames = data['測項'][:18].to_list()
print(indexNames)
drop  = ['RAINFALL','WIND_DIREC','NO2','THC']

for item in drop:
    index = data[ data['測項'] == item ].index
...
m = data.mean(axis=1,skipna=True)
...
for month in range(12):
    sample = np.empty([14, 480])
    for day in range(20):
        sample[:, day * 24 : (day + 1) * 24] = raw_data[14 * (20 * month + day) : 14 * (20 * month + day + 1), :]
        month_data[month] = sample

for i in range(5):
    month_data[6+i] = month_data[7+i]
...
"""valid = []
    ...
    if month_data[month][8, day * 24 + hour] ==-1:
    hours-=1
    valid.append(hours)
    print(valid)"""

dim = 11*471
x = np.empty([dim, 14 * 9], dtype = float)
y = np.empty([dim, 1], dtype = float)
#print(month_data)
...
testdata = pd.read_csv('hw1_data/test.csv', header = None, encoding = 'big5')
'''testdata = testdata.drop(testdata[testdata[1]=='THC'].index)
testdata = testdata.drop(testdata[testdata[1]=='RAINFALL'].index)
testdata = testdata.drop(testdata[testdata[1]=='NO2'].index)
testdata = testdata.drop(testdata[testdata[1]=='WIND_DIREC'].index)'''
for item in drop:
    index = testdata[ testdata[1] == item ].index
    testdata = testdata.drop(index)
